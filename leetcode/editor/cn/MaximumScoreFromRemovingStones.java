...
After that, there are fewer than two non-empty piles, so the game ends.
 ...
 Related Topics 贪心 数学 堆（优先队列） 👍 46 👎 0
...
public class MaximumScoreFromRemovingStones{
    public static void main(String[] args) {
        ...
        System.out.println("solution.maximumScore(1, 8, 8) = " + solution.maximumScore(1, 8, 8));
    }
...
class Solution {
    public int maximumScore(int a, int b, int c) {
        int[] list = {a, b, c};
        Arrays.sort(list);
        if (list[0] + list[1] <= list[2]) return list[0] + list[1];
        return Arrays.stream(list).sum() / 2;
    }
}
