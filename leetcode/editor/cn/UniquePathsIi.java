...
Output: 1
 ...
 Related Topics 数组 动态规划 矩阵 👍 784 👎 0
...
//        int[][] dp = new int[m][n];
        ...
        int tag1 = 0;
        for (int i = 0; i < m; i++) {
            if (obstacleGrid[i][0] == 1 && tag1 == 0) {
                tag1 = 1;
            }
            if (tag1 == 1) obstacleGrid[i][0] = 1;
        }
        int tag2 = 0;
        for (int i = 0; i < n; i++) {
            if (obstacleGrid[0][i] == 1 && tag2 == 0) tag2 = 1;
            if (tag2 == 1) obstacleGrid[0][i] = 1;
